<template>
	<div>
		...
		<div class="two-col-table-header" :class="isActive ? '' : 'bottom-border'">
			<span @click="isActive = !isActive">{{ tableName }}</span>
			<span class="arrow" :class="isActive ? 'show' : 'hide'"></span>
		</div>
		
		<div class="two-col-table" v-if="isActive">
			<div class="two-col-item" v-for="row in tableRows" :class="row['postprocessors'] ? 'postprocessors' : ''">
				<template v-for="(value, key) in row">
					
					<template v-if="key != 'postprocessors'">
						<span>{{ key | capitalizeFirst }}:</span><span>{{ value }}</span>
					</template>
					...
					<template v-else>
						...
						<div>
							...
							<template v-for="(item, index) in value">
								<span>{{ item}}</span><span v-if="index < (value.length - 1)">, </span>
							</template>
						</div>
